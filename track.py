...
def run(
        ...
        save_dataset=False,  # save results to *.txt
    ...
    (save_dir / 'tracks' if save_dataset else save_dir).mkdir(parents=True, exist_ok=True)  # make dir
    ...
    if save_dataset or save_vid:
        s = f"\n{len(list(save_dir.glob('tracks/*.txt')))} tracks saved to {save_dir / 'tracks'}" if save_dataset else ''
...
def parse_opt():
    ...
    parser.add_argument('--save-dataset', action='store_true', help='save results to *.txt')
